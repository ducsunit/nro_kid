...
public class Panel : IActionListener, IChatable
{
		...
		if (GameCanvas.isPointerDown)
		{
			...
			else if (pointerIsDowning)
			{
				...
				if (pointerDownTime > 5 && pointerDownFirstX == GameCanvas.py && !isDownWhenRunning)
				{
                    ...
                    if (isTabInven() && (cmtoY + GameCanvas.py - yScroll) / ITEM_HEIGHT > Char.myCharz().arrItemBody.Length - 1)
                    {
                        var row = ((cmtoY + GameCanvas.py - yScroll) / ITEM_HEIGHT) - Char.myCharz().arrItemBody.Length + 1;
                        var col = (GameCanvas.px - xScroll) / 33;
                        if (position == 1)
                        {
                            col = (GameCanvas.px - xScroll) / 33;
                        }
                        selected = (row * 5 + col) + (Char.myCharz().arrItemBody.Length - 5);
                    }
                    else
                    {
	...
	private void setTabInventory(bool resetSelect)
	{
        ...
        currentListLength = arrItemBody.Length + arrItemBag.Length / 5;
        cmyLim = currentListLength * ITEM_HEIGHT - hScroll;
        cmy = (cmtoY = cmyLast[currentTabIndex]);
        if (cmyLim < 0)
        {
    ...
    private void paintInventory(mGraphics g)
    {
        ...
        {
            g.setColor(16711680);
            g.setClip(xScroll - 1, yScroll, wScroll, hScroll);
            g.translate(0, -cmy);
            Item[] arrItemBody = global::Char.myCharz().arrItemBody;
            Item[] arrItemBag = global::Char.myCharz().arrItemBag;
                        ...
                        if (item.itemOption != null)
                        {
                            for (int l = 0; l < item.itemOption.Length; l++)
                            {
                                if (item.itemOption[l].optionTemplate.id == 72)
                                {
                                    ...
                                    item.paintColorItem(g, num6 + 18, num7 + 12, item.itemOption[l].param);
                                }
                                if (item.itemOption[l].optionTemplate.id == 41)
                                {
                                    if (item.itemOption[l].param == 1)
            ...
            for (int k = 0; k < arrItemBag.Length; k++)
            {
                    int columns = 5;
                    int row = k / columns;
                    int col = k % columns;
                    int xscroll = xScroll + col * ((wScroll - 5 + 5) / columns) + 1;
                    int yscroll = yScroll + arrItemBody.Length * ITEM_HEIGHT - 10 + row * ITEM_HEIGHT + 10;
                    int x = xscroll;
                    int y = yscroll;
                    int w = 34;
                    int h = ITEM_HEIGHT - 1;
                    if (yscroll - cmy <= yScroll + hScroll && yscroll - cmy >= yScroll - ITEM_HEIGHT)
                    {
                        g.setColor(11837316);
                        g.fillRect(x, y, w, h);
                        Item item2 = arrItemBag[k];
                        if (item2 != null)
                        {
                            g.setColor(11837310);
                            g.fillRect(x + 1, y + 1, w - 2, h - 2);
                            SmallImage.drawSmallImage(g, (int)item2.template.iconID, x + w / 2, y + h / 2, 0, 3);
                            if (item2.quantity > 1)
                            {
                                mFont.tahoma_7b_white.drawString(g, item2.quantity.ToString(), x + w, y + h - mFont.tahoma_7_yellow.getHeight(), 1);
                            }
                        if (k + arrItemBody.Length == selected)
                        {
	...
	private void doFireInventory()
	{
        ...
        {
            Item item2 = global::Char.myCharz().arrItemBody[selected];
            if (item2 != null)
            {
                currItem = item2;
        ...
        if (currItem != null)
        {
            global::Char.myCharz().setPartTemp(currItem.headTemp, currItem.bodyTemp, currItem.legTemp, currItem.bagTemp);
    ...
    public void ModLoChatTextField(string strChat, string strName)
    {
        if (chatTField == null)
        {
            chatTField = new ChatTextField();
    ...
    public void perform(int idAction, object p)
	{
        ...
        if (idAction == 8010)
        {
            ModLoChatTextField("Nhập Số Sao Cần Đập", "Chỉ Được Nhập Số");
        }
        if (idAction == 7001)
		{
			Item item7 = (Item)p;
