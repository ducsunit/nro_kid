public class Item
{
    public void paintColorItem(mGraphics g, int x, int y, int param)
    {
        if (param > 7)
        {
            param = 7;
        ...
        if (param < 4)
        {
            if (param == 1)
            {
                for (int i = 0; i < 2; i++)
                {
                    ...
                    {
                        int num = x - 17 + this.u(GameCanvas.gameTick + array4[i] - j * 4);
                        int num2 = y - 12 + this.v(GameCanvas.gameTick + array5[i] - j * 4);
                        g.setColor(this.colorBorder[array[param - 1]][j]);
                        g.fillRect(num - this.size[j] / 2, num2 - this.size[j] / 2, this.size[j], this.size[j]);
                    }
                }
                return;
            }
            if (param != 2)
            {
                for (int k = 0; k < 2; k++)
                {
                    ...
                    {
                        int num3 = x - 17 + this.u(GameCanvas.gameTick + array4[k] - l * 4);
                        int num4 = y - 12 + this.v(GameCanvas.gameTick + array5[k] - l * 4);
                        g.setColor(this.colorBorder[0][l]);
                        g.fillRect(num3 - this.size[l] / 2, num4 - this.size[l] / 2, this.size[l], this.size[l]);
                    }
                }
                for (int m = 2; m < 4; m++)
                {
                    ...
                    {
                        int num5 = x - 17 + this.u(GameCanvas.gameTick + array4[m] - n * 4);
                        int num6 = y - 12 + this.v(GameCanvas.gameTick + array5[m] - n * 4);
                        g.setColor(this.colorBorder[1][n]);
                        g.fillRect(num5 - this.size[n] / 2, num6 - this.size[n] / 2, this.size[n], this.size[n]);
                    }
                }
                return;
            }
        ...
        for (int num7 = 0; num7 < 4; num7++)
        {
            ...
            {
                int num9 = x - 17 + this.u(GameCanvas.gameTick + array4[num7] - num8 * 4);
                int num10 = y - 12 + this.v(GameCanvas.gameTick + array5[num7] - num8 * 4);
                g.setColor(this.colorBorder[array[param - 1]][num8]);
                g.fillRect(num9 - size[num8] / 2, num10 - size[num8] / 2, size[num8], this.size[num8]);
            }
        }
    }
    private int u(int a)
    ...
    // Token: 0x0600033F RID: 831 RVA: 0x0001BFB8 File Offset: 0x0001A1B8
    private int v(int a)
    {
        int num = 22;
        int num2 = a % 88;
