...
namespace Mod
{
	public class MainMod : IActionListener, IChatable
	{
		...
		public static void Paint(mGraphics g)
		{
            ...
            mFont.drawString(g, "Map: " + TileMap.mapNames[TileMap.mapID] + " [" + TileMap.zoneID + "]", 25, GameCanvas.h - 220, 0, mFont2);
			mFont.drawString(g, "Time: " + DateTime.Now.ToString("HH:mm:ss dd/MM/yyyy"), 25, GameCanvas.h - 210, 0, mFont2);
			int num = GameCanvas.h - 200;
			if (AutoSkill.isAutoSendAttack)
			{
				mFont.drawString(g, "Tự đánh: on", 25, num, 0, mFont2);
				num += 10;
			}
			if (isAutoRevive)
			{
				mFont.drawString(g, "Auto hồi sinh: on", 25, num, 0, mFont2);
				num += 10;
			}
			if (AutoPick.isAutoPick)
			{
				mFont.drawString(g, "Auto nhặt: on", 25, num, 0, mFont2);
				num += 10;
			}
			if (isLockFocus)
			{
				mFont.drawString(g, "Khóa: " + charIDLock, 25, num, 0, mFont2);
				num += 10;
			}
			if (isAutoEnterNRDMap)
			{
				mFont.drawString(g, "Đang auto nrd: " + mapIdNRD + "sk" + zoneIdNRD, 25, num, 0, mFont2);
				num += 10;
			}
		}
		...
		public static bool UpdateKey(int unused)
		{
			if (GameCanvas.keyAsciiPress == Hotkeys.A)
			{
				...
				GameScr.info1.addInfo("Tự Đánh\n" + (AutoSkill.isAutoSendAttack ? "[ON]" : "[OFF]"), 0);
				return true;
			}
			...
			if (GameCanvas.keyAsciiPress == Hotkeys.P)
			{
				...
				GameScr.info1.addInfo("Kết Nối\n" + (isConnectToAccountManager ? "[ON]" : "[OFF]"), 0);
				return true;
			}
		...
		{
			MyVector myVector = new MyVector();
			myVector.addElement(new Command("Thông Báo\nBoss\n" + (isHuntingBoss ? "[ON] " : "[OFF]"), getInstance(), 29, null));
			myVector.addElement(new Command("Danh Sách\nNhân Vật Trong Khu\n" + (isShowCharsInMap ? "[ON]" : "[OFF]"), getInstance(), 30, null));
			GameCanvas.menu.startAt(myVector, 3);
		}
		...
		public static bool Chat(string text)
		{
			...
			if (text.StartsWith("k "))
			{
				try
				{
					int zoneId = int.Parse(text.Split(' ')[1]);
			...
			if (text.StartsWith("s "))
			{
				try
				{
					int num = (runSpeed = int.Parse(text.Split(' ')[1]));
			...
			if (text.StartsWith("cheat "))
			{
				try
				{
					float num5 = (Time.timeScale = float.Parse(text.Split(' ')[1]));
			...
			if (text.StartsWith("cheatf "))
			{
				try
				{
					float num7 = (Time.timeScale = float.Parse(text.Split(' ')[1]) / 10f);
		...
		public static void LoadData()
		{
			...
			if (mGraphics.zoomLevel == 2)
				try
				{
					logoGameScreen = Image.createImage(Convert.FromBase64String(LogoMod.logoKoiX2Base64));
					logoServerListScreen = Image.createImage(Convert.FromBase64String(LogoMod.logoKoiServerListScreenBase64));
				}
				catch
				{
				}
			if (mGraphics.zoomLevel == 1)
				...
				{
					logoServerListScreen = Image.createImage(Convert.FromBase64String(LogoMod.logoKoiServerListScreenBase64));
					logoGameScreen = Image.createImage(Convert.FromBase64String(LogoMod.logoKoiX1Base64));
				}
				catch
				{
				}
