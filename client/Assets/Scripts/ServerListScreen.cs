...
public class ServerListScreen : mScreen, IActionListener
{
	...
	public override void paint(mGraphics g)
	{
		...
		int num2 = 2;
		mFont.tahoma_7b_white.drawString(g, "v" + GameMidlet.VERSION + "(" + mGraphics.zoomLevel + ")", GameCanvas.w - 2, num2 + 15, 1, mFont.tahoma_7_grey);
		if (!isGetData || loadScreen)
		{
			if (mSystem.clientType == 1 && !GameCanvas.isTouch)
			{
				mFont.tahoma_7b_white.drawString(g, linkweb, GameCanvas.w - 2, GameCanvas.h - 15, 1, mFont.tahoma_7_grey);
			}
			else
			{
				mFont.tahoma_7b_white.drawString(g, linkweb, GameCanvas.w - 2, num2, 1, mFont.tahoma_7_grey);
			}
		}
		else
		{
		...
		if (cmdDeleteRMS != null)
		{
			mFont.tahoma_7b_white.drawString(g, mResources.xoadulieu, GameCanvas.w - 2, GameCanvas.h - 15, 1, mFont.tahoma_7_grey);
		}
	...
	{
		smartPhoneVN = "NroDK:nrodk.online:14445:0,0,0";
		javaVN = smartPhoneVN;
		smartPhoneIn = smartPhoneVN;
		javaIn = smartPhoneVN;
		smartPhoneE = smartPhoneVN;
		javaE = smartPhoneVN;
