...
public class Service {
    ...
    private short clampToShort(long value) {
    ...
    public void Send_Info_NV(Player pl) {
        ...
        try {
            ...
            msg.writer().writeInt(clampToInt(pl.nPoint.hp));
            msg.writer().writeByte(0);//Hiệu ứng Ăn Đậu
            msg.writer().writeInt(clampToInt(pl.nPoint.hpMax));
...
//        }
    ...
    public void point(Player player) {
        ...
        if (!player.isPet && !player.isBoss && !player.isBot) {
            ...
            try {
                msg = new Message(-42);
                // Keep -42 packet types aligned with legacy client reader (int/int/int...).
                msg.writer().writeInt(clampToInt(player.nPoint.hpg));
                msg.writer().writeInt(clampToInt(player.nPoint.mpg));
                msg.writer().writeInt(clampToInt(player.nPoint.dameg));
                msg.writer().writeInt(clampToInt(player.nPoint.hpMax));// hp full
                msg.writer().writeInt(clampToInt(player.nPoint.mpMax));// mp full
                msg.writer().writeInt(clampToInt(player.nPoint.hp));// hp
                msg.writer().writeInt(clampToInt(player.nPoint.mp));// mp
                ...
                msg.writer().writeShort(clampToShort(player.nPoint.defg));
                msg.writer().writeByte(clampToByte(player.nPoint.critg));
                player.sendMessage(msg);
                msg.cleanup();
            } catch (Exception e) {
                Log.error(Service.class, e);
            ...
            for (Item item : itemsBody) {
                if (!item.isNotNullItem()) {
                    ...
                    for (ItemOption itemOption : itemOptions) {
                        ...
                        msg.writer().writeShort(clampToShort(itemOption.param));
                    }
                }
...
//    }
    ...
    public void hsChar(Player pl, long hp, long mp) {
        ...
        try {
            ...
            msg.writer().writeInt((int) pl.id);
            msg.writer().writeInt(clampToInt(hp));
            msg.writer().writeInt(clampToInt(pl.nPoint.hpMax));
    ...
    public void charDie(Player pl) {
        ...
        try {
            if (!pl.isPet) {
                msg = new Message(-17);
                msg.writer().writeByte(pl.cFlag); // cPk for self-death packet
                msg.writer().writeShort(pl.location.x);
                msg.writer().writeShort(pl.location.y);
                msg.writer().writeLong(pl.nPoint.power); // optional field client reads in try-catch
                pl.sendMessage(msg);
                msg.cleanup();
            } else {
...
//            Send_Info_NV(pl);
    ...
    public void sendTitleRv(Player player, Player p2, int id) {
        ...
        try {
            ...
            me.writer().writeShort(getTitleEffectId(player, id));
    ...
    private short getTitleEffectId(Player player, int id) {
        if (id >= 1470 && id <= 1478) {
            return (short) (91 + (id - 1470));
        }
        if (id == 888 && player.lastTimeTitle1 > 0 && player.isTitleUse1) {
            return 89;
        }
        if (id == 889 && player.lastTimeTitle2 > 0 && player.isTitleUse2) {
            return 237;
        }
        if (id == 890 && player.lastTimeTitle3 > 0 && player.isTitleUse3) {
            return 88;
        }
        if (id == 891 && player.lastTimeTitle4 > 0 && player.isTitleUse4) {
            return 236;
        }
        if (id == 892 && player.lastTimeTitle5 > 0 && player.isTitleUse5) {
            return 235;
        }
        if (id == 900 && player.nPoint.power >= 500_000_000_000L) {
            if (player.gender == 0) {
                return 94;
                ...
                for (Item item : pl.pet.inventory.itemsBody) {
                    if (!item.isNotNullItem()) {
                        ...
                        for (ItemOption iop : itemOptions) {
                            ...
                            msg.writer().writeShort(clampToShort(iop.param));
                        }
                    }
                }
...
//                msg.writer().writeLong(pl.pet.nPoint.defg); //mpfull
//                msg.writer().writeInt(pl.pet.nPoint.critg); //mpfull
                // Keep pet info payload aligned with client parser in Controller(-107).
                msg.writer().writeInt(clampToInt(pl.pet.nPoint.hp)); //hp
                msg.writer().writeInt(clampToInt(pl.pet.nPoint.hpMax)); //hpfull
                msg.writer().writeInt(clampToInt(pl.pet.nPoint.mp)); //mp
                msg.writer().writeInt(clampToInt(pl.pet.nPoint.mpMax)); //mpfull
                msg.writer().writeInt(clampToInt(pl.pet.nPoint.dame)); //damefull
                ...
                msg.writer().writeByte(clampToByte(pl.pet.nPoint.crit)); //crit
                msg.writer().writeShort(clampToShort(pl.pet.nPoint.def)); //def
...
//            Service.getInstance().sendMessAllPlayerInMap(pl.map, msg);
    ...
    public void sendPlayerInfo(Player player) {
        try {
            ...
            msg.writer().writeInt(clampToInt(player.nPoint.hp));
            msg.writer().writeInt(clampToInt(player.nPoint.hpMax));
